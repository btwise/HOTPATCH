// battery
// In config ACPI, H_EC renamed EC
// Find:     48 5F 45 43
// Replace:  45 43 5F 5F
//
...
DefinitionBlock ("", "SSDT", 2, "hack", "BATT", 0)
{
    ...
    External(_SB.PCI0.LPCB.EC.BST1, FieldUnitObj)
    ...
    {      
                    Method (BTIF, 1, Serialized)
                    {
                        ...
                        If (\_SB.PCI0.LPCB.EC.ECRG)
                        {
                            ...
                            Local1 = (Local0 * \_SB.PCI0.LPCB.EC.NLB1)
                            Divide ((Local1 + 0x63), 0x64, Local3, Local2)
                            DerefOf (NBTI [Arg0]) [0x05] = Local2
                            DerefOf (NBTI [Arg0]) [0x05] = \_SB.PCI0.LPCB.EC.NLB1
                            DerefOf (NBTI [Arg0]) [0x06] = \_SB.PCI0.LPCB.EC.NLB2
                            Switch (Arg0)
                            {
                                Case (Zero)
                                {
    ...
    Scope(\_SB.PCI0.LPCB.EC)
    {
                    Method (BTST, 2, Serialized)
                    {
                        ...
                        If (Arg1)
                        {
                            \_SB_.PCI0.LPCB.EC.NGBT = 0xFF
                        }

                        Local0 = \_SB_.PCI0.LPCB.EC.NGBT
                        Release (BTMX)
                        If (((Local0 & Local7) == Zero))
                        {
                            Return (Zero)
